    ...
    export async function manageHandler(request: Express.Request, response: Express.Response, next: Express.NextFunction) {
        try {
            if(!Module.Utils.Express.validateParameter(request, [
                { path: [ 'uuid' ], name: 'Account uuid', type: Module.Utils.Express.ParameterTypes.string, isRequired: true },
            ...
            await Plugin.Resources.getDatabaseClient().manager.getRepository(Plugin.Resources.Model.Users).update({ uuid: request.body.uuid }, { expires_date: new Date(request.body.expiresDate) })
